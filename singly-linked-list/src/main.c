...
static struct Node * node_push(struct Node *node_prev, uint16_t *data)
{
...
{
	struct Node *prev_node = node_head, *node, *next_node;	
	for(uint16_t i = 0; index != i && i < 0xffff; ++i){
...
{
	struct Node *node = node_head->fd;
	for(uint16_t i = 0; node != (void*)0x00; ++i){
		printf("index: %d\n", i);
		printf("fd: %p\n", node->fd);
		printf("addr: %p\n", node);
		printf("data: %s\n\n", node->data);
...
int main()
{
	...
	uint16_t data[10], shit;

	for(uint16_t i = 0; i < 5; ++i) {
		sprintf(&data, "fuck%d", i);
